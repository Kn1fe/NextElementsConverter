...
namespace NextElementsConverter.ElementsFile
{
    public class eListCollection
    {
        MainForm form;

        public eListCollection(string elFile, MainForm form)
        {
            this.form = form;
            Lists = Load(elFile);
        }
        public eListCollection(MainForm form)
        {
            this.form = form;
        }
        ...
        private void writeValue(BinaryWriter bw, object value, string type)
        {
            if (type == "int16")
            {
                bw.Write(short.Parse(value.ToString()));
                return;
            }
            if (type == "int32")
            {
                bw.Write(int.Parse(value.ToString()));
                return;
            }
            if (type == "int64")
            {
                bw.Write(Int64.Parse(value.ToString()));
                return;
            }
            if (type == "float")
            {
                bw.Write(float.Parse(value.ToString()));
                return;
            }
            if (type == "double")
            {
                bw.Write(double.Parse(value.ToString()));
                return;
            }
            if (type.Contains("byte:"))
            {
        ...
        public eList[] loadConfiguration(string file)
        {
        ...
        public eList[] Load(string elFile)
        {
            ...
            if (configFiles.Length > 0)
            {
                ...
                form.Progress.Maximum = Li.Length;
        ...
        {
            form.Progress.Visible = true;
            form.Progress.Maximum = Lists.Length;
